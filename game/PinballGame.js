    ...
    checkLoadingComplete() {
        ...
        if (allLoaded) {
            setTimeout(async () => {
                ...
                // КРИТИЧЕСКИ ВАЖНО: Вызываем ready() только ПОСЛЕ полной загрузки
                if (window.farcasterManager) {
                    console.log('🎮 Game fully loaded, notifying Farcaster that app is ready...');
                    await window.farcasterManager.notifyAppReady();
                }
